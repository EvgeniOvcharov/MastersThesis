...
---
# Data Processing
We load football data from a Kaggle containing Spanish League results(https://www.kaggle.com/datasets/ricardomoya/football-matches-of-spanish-league), for the 2016-17 and 2017-18 seasons.
...
ln(\tau_\lambda,_\mu(x,y)) = \begin{cases}
            ...
            ln(\mu) + ln(\rho0         \quad \quad\quad\ \ \ \ \text{if } x=1, y=0\\
            -ln(\rho)            \qquad \quad \quad\quad\ \ \text{if } x=y=1\\
            0                  \qquad \qquad \qquad \qquad \ \text{otherwise}
            \end{cases}
$$
Where $\lambda$, $\beta$, $\gamma$ are the log-values of the versions of the previously defined parameters.

In what follows we apply the same constraint optimization algorithm as before to the same likelihood function but using the redefined parameters. The previously used constraint is now transformed to:
...
S_DC2
```
We rank teams based on their overall ability $\alpha_i + \beta_i$ of attack and defence, represented on the multiplicative scale as $e^{\alpha_i + \beta_i}$ .
...
```
Four of the top six teams are actually among the weakest teams in the league. 

## Maher's iterative approach

Implemetnitng Maher's original model with four parameters.

Calculating alpha and beta based on "home scores". Results after first itteration.
...
local <- x_balanced %>%
  ...
  summarise(localGoals_local = sum(localGoals),
            alpha_0 = sum(localGoals)/sqrt(Sx))
visitor <- x_balanced %>%
  group_by(visitor_team_index) %>%
  summarise(localGoals_visitor = sum(localGoals),
            beta_0 = sum(localGoals)/sqrt(Sx))

it_data <- left_join(local, visitor, by = c("local_team_index" = "visitor_team_index"))

for (i in 1:nrow(it_data)){
  it_data$beta_0[i] <- it_data$localGoals_visitor[i]/(sum(it_data$alpha_0)- it_data$alpha_0[i])
}

for (i in 1:nrow(it_data)){
  it_data$alpha_0[i] <- it_data$localGoals_local[i]/(sum(it_data$beta_0)- it_data$beta_0[i])
}

head(it_data)
```

Results after second itteration.
```{r}
for (i in 1:nrow(it_data)){
  it_data$beta_0[i] <- it_data$localGoals_visitor[i]/(sum(it_data$alpha_0)- it_data$alpha_0[i])
}
...
Results after the third iteration are the same as after the second one so these are our true values for $\alpha$ - attack streanght when playing at home and $\beta$ - weakness of defense when playing away.
```{r}
for (i in 1:nrow(it_data)){
  it_data$beta_0[i] <- it_data$localGoals_visitor[i]/(sum(it_data$alpha_0)- it_data$alpha_0[i])
}
...
Ranking teams based on $\alpha$ + $\beta$
```{r}
alpha <- it_data$alpha_0
beta <- it_data$beta_0
teams$attack <- alpha
teams$defence <- beta
#teams <- select(teams,-alpha,-beta)
...
local <- x_balanced %>%
  ...
  summarise(visitorGoals_local = sum(visitorGoals),
            gamma_0 = sum(visitorGoals)/sqrt(Sy))
visitor <- x_balanced %>%
  group_by(visitor_team_index) %>%
  summarise(visitorGoals_visitor = sum(visitorGoals),
            delta_0 = sum(visitorGoals)/sqrt(Sy))

it_data2 <- left_join(local, visitor, by = c("local_team_index" = "visitor_team_index"))

for (i in 1:nrow(it_data2)){
  it_data2$delta_0[i] <- it_data2$visitorGoals_visitor[i]/(sum(it_data2$gamma_0)- it_data2$gamma_0[i])
}

for (i in 1:nrow(it_data2)){
  it_data2$gamma_0[i] <- it_data2$visitorGoals_local[i]/(sum(it_data2$delta_0)- it_data2$delta_0[i])
}
...
Again the results after the third iteration are the same as after the second one so these are our true values for \delta - attack streanght when playing at away and \gamma - weakness of defense when playing at home.
...
Ranking teams based on \gamma + \delta
```{r}
gamma <- it_data2$gamma_0
delta <- it_data2$delta_0
teams$attack <- delta
teams$defence <- gamma
#teams <- select(teams,-alpha,-beta)
game_statistics_mod4 <- left_join(game_statistics,teams, by = c("team"))
game_statistics_mod4 <- game_statistics_mod4 %>%
  mutate(skill = exp(attack*(1/defence))) %>%
  arrange(desc(skill))
head(game_statistics_mod4)

```

Comparing the actual games outcomes with the ones predicted by the model
```{r}
verify_model <- left_join(it_data,it_data2, by = "local_team_index")
verify_model <- verify_model[,c(1,3,5,7,9)]
verify_model <- verify_model %>%
  rename(team_index = local_team_index)
x_updated <- left_join(x,verify_model[,c("team_index","alpha_0","gamma_0")], by = c("local_team_index" = "team_index"))
x_updated <- left_join(x_updated,verify_model[,c("team_index","beta_0","delta_0")], by = c("visitor_team_index" = "team_index"))
x_updated <- x_updated %>%
  mutate( x_score_pred = round(alpha_0*beta_0,0),
          y_score_pred = round(delta_0*gamma_0,0))
